...
def run_xvfb():
    ...
    print(server_id, auth_file_id)
    xauthority_path = '/tmp/Xauthority_{}'.format(auth_file_id)

    command = '"{}" -s "-screen 0 1400x900x24" -n {} -f {}'.format(SCRIPT_PATH, server_id, xauthority_path)
...
echo $SERVERNUM >/tmp/x_server_id
echo $AUTHFILE >>/tmp/x_server_id
